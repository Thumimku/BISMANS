            ...
            <div class = "w3-agile-pane">
                ...
                <div class="stats-info stats-last widget-shadow">
                    <table class="table stats-table ">
                        ...
                        <tbody>
                        <?php
                        $inventoryQuery = "SELECT * from tblinventory order BY quantity asc LIMIT 5";
                        $inventoryQuery = $mysqli->query($inventoryQuery);

                        while($inventoryObj = $inventoryQuery->fetch_object()){
                            $sku = $inventoryObj->sku;
                            $productId = $inventoryObj->productId;
                            $availQuantity = $inventoryObj->quantity;
                            $productQuery = "SELECT * from tblproductdetails WHERE productId = $productId";
                            $productQuery = $mysqli->query($productQuery);
                            $productObj = $productQuery->fetch_object();
                            $productName = $productObj->name;
                            $threshold = $productObj->threshold;
                            $supplier = $productObj->supplier;
                            $supplierQuery = "SELECT contactNo from tblsupplierdetails WHERE supplierName ='".$supplier."'";
                            $supplierQuery = $mysqli->query($supplierQuery);
                            $supplierObj  = $supplierQuery->fetch_object();
                            $contactNo = $supplierObj->contactNo;
                            $availability = round(($availQuantity-$threshold)*100/$threshold)."%";
